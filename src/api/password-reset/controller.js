...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your Web Api Tuts account.<br>
        ...
        &mdash; Web Api Tuts Team
      `
      return sendMail({ toEmail: email, subject: 'Web Api Tuts - Password Reset', content })
    })
